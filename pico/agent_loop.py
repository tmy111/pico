...
class AgentLoop:
    ...
    def run(self, user_message):
        ...
        while tool_steps < agent.max_steps and attempts < max_attempts:
            ...
            try:
                ...
                agent.emit_trace(
                    ...
                    "model_error",
                    {
                        "error": clip(final, 500),
                        "completion_metadata": completion_metadata,
                        "duration_ms": int((time.monotonic() - model_started_at) * 1000),
                    },
                )
                checkpoint = agent.create_checkpoint(task_state, user_message, trigger=task_state.stop_reason)
                agent.run_store.write_task_state(task_state)
                agent.emit_trace(
                    task_state,
                    "checkpoint_created",
                    {
                        "checkpoint_id": checkpoint["checkpoint_id"],
                        "trigger": task_state.stop_reason,
                    },
                )
                agent.emit_trace(
                    task_state,
        ...
        if tool_steps >= agent.max_steps and attempts < max_attempts:
            ...
            task_state.record_attempt()
            finalization_notice = (
                f"Runtime notice: tool step budget is exhausted ({tool_steps}/{agent.max_steps}). "
                "Do not call more tools. Return exactly one non-empty <final> answer using only the information already gathered. "
                "Mention any important files or areas you did not inspect."
            )
            agent.record({"role": "assistant", "content": finalization_notice, "created_at": now()})
            agent.run_store.write_task_state(task_state)
            prompt_started_at = time.monotonic()
            prompt, prompt_metadata = agent._build_prompt_and_metadata(user_message)
            prompt_metadata["finalization_turn"] = True
            agent.emit_trace(
                task_state,
                "prompt_built",
                {
