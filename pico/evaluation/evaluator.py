...
def _run_verifier(command, cwd):
    ...
    Benchmark fixtures use portable POSIX-style commands such as
    ``python3 -c \"... Path('README.md') ...\"``. Windows' ``cmd.exe`` does
    not treat single quotes as quotes, so ``shell=True`` can corrupt the Python
    program before it runs. Splitting once preserves it as a single argument on
    each platform. Python aliases are resolved to this harness' interpreter to
    keep a virtual environment reproducible.
    """
