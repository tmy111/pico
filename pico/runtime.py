...
# Pico 是 agent 运行时主体，负责模型循环、工具执行、记忆和工件记录。
class Pico:
    def __init__(
        self,
        model_client,
...
>>>>>>> origin/main
    ...
    def refresh_prefix(self, force=False):
        # 当工作区状态变化时，刷新 prefix 和 workspace snapshot。
    ...
    def history_text(self):
        # 把 session history 压成一段可读文本，主要给子 agent/记忆使用。
...
<<<<<<< HEAD
    ...
    def set_skill(self, name):
        skill = skilllib.require_skill(name)
...
>>>>>>> origin/main
    ...
    def prompt_metadata(self, user_message, prompt):
        # 返回这次 prompt 的追踪元信息，CLI 或测试可以单独查看。
        _, metadata = self._build_prompt_and_metadata(user_message)
        return metadata

    def _build_prompt_and_metadata(self, user_message):
        # 组装模型真正收到的 prompt，并同步更新恢复状态、记忆视图和缓存元数据。
...
>>>>>>> origin/main
    ...
    def emit_trace(self, task_state, event, payload=None):
        # 写入一条运行过程事件，用来复盘 agent 每一步发生了什么。
    ...
    def capture_workspace_snapshot(self):
        # 扫描工作区文件并记录摘要，checkpoint 用它判断恢复时文件是否变过。
        snapshot = {}
    ...
    def extract_durable_promotions(self, user_message, final_answer):
        # 当用户明确说“记住/保存”时，从最终回答里抽取可沉淀的长期记忆。
...
<<<<<<< HEAD
    def redundant_tool_call(self, name, args):
        ...
        if name == "read_file":
            return self.redundant_read_file_call(args)
        return None
    ...
    @staticmethod
    def format_line_ranges(ranges):
        return ", ".join(f"{start}-{end}" if start != end else str(start) for start, end in ranges)

    def redundant_read_file_call(self, args):
        path, start, end = self.read_file_span(args)
        prior_ranges = []
        for item in self.current_request_tool_events():
            if item.get("name") != "read_file":
                continue
            try:
                prior_path, prior_start, prior_end = self.read_file_span(item.get("args", {}))
            except Exception:
                continue
            if prior_path == path:
                prior_ranges.append((prior_start, prior_end))
        if not prior_ranges:
            return None

        merged = self.merge_line_ranges(prior_ranges)
        requested_length = end - start + 1
        covered = 0
        for prior_start, prior_end in merged:
            overlap_start = max(start, prior_start)
            overlap_end = min(end, prior_end)
            if overlap_start <= overlap_end:
                covered += overlap_end - overlap_start + 1

        ranges_text = self.format_line_ranges(merged)
        if covered >= requested_length:
            return (
                "repeated_read_covered",
                f"error: redundant read_file for {path} lines {start}-{end}; already read lines {ranges_text}. "
                "Choose a different file/range or return a final answer.",
            )
        if covered / max(requested_length, 1) >= 0.8:
            return (
                "repeated_read_overlap",
                f"error: overlapping read_file for {path} lines {start}-{end}; already read lines {ranges_text}. "
                "Read only new lines, choose another file, or return a final answer.",
            )
        return None
...
>>>>>>> origin/main
    ...
    def approve(self, name, args):
        # 根据只读模式和 approval_policy 判断某个工具调用是否允许执行。
        if self.read_only:
            return False
...
>>>>>>> origin/main
    ...
    def reset(self):
        # 清空当前 session 的历史和记忆，但保留 session 文件本身。
