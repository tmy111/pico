...
def test_agent_retries_after_untagged_model_text(tmp_path):
    agent = build_agent(
        ...
        [
            "我们需要先继续读取文件，然后可以",
            "<final>Recovered after untagged reasoning.</final>",
        ],
    )

    answer = agent.ask("Summarize the repository")

    assert answer == "Recovered after untagged reasoning."
    notices = [item["content"] for item in agent.session["history"] if item["role"] == "assistant"]
    assert any("untagged text" in item for item in notices)
    assert not any(item["content"] == "我们需要先继续读取文件，然后可以" for item in agent.session["history"])


def test_parse_rejects_untagged_text_as_retry():
    kind, payload = Pico.parse("这是没有 final 标签的普通文本")
...
def test_redundant_read_file_range_is_rejected(tmp_path):
    ...
    agent = build_agent(tmp_path, [])
    agent.record({"role": "user", "content": "inspect hello", "created_at": "0"})
    agent.record(
        {
            "role": "tool",
            "name": "read_file",
            "args": {"path": "hello.txt", "start": 1, "end": 10},
            "content": "already read",
            "created_at": "1",
        }
    )

    result = agent.run_tool("read_file", {"path": "hello.txt", "start": 3, "end": 8})

    assert "error: redundant read_file for hello.txt lines 3-8" in result
    assert "already read lines 1-10" in result


def test_overlapping_read_file_range_is_rejected(tmp_path):
    ...
    result = agent.run_tool("read_file", {"path": "hello.txt", "start": 5, "end": 24})

    assert "error: overlapping read_file for hello.txt lines 5-24" in result
...
def test_deepseek_client_accepts_gateway_text_shapes():
    payloads = [
        ...
        {"completion": "<final>completion string</final>"},
        {"choices": [{"message": {"content": "<final>choice string</final>"}}]},
        {"choices": [{"message": {"content": [{"text": "<final>choice text block</final>"}]}}]},
        {"choices": [{"message": {"content": ""}}, {"message": {"content": "<final>second choice</final>"}}]},
        {"choices": [{"delta": {"content": "<final>delta string</final>"}}]},
    ]
...
def test_deepseek_client_rejects_reasoning_only_response():
    class FakeResponse:
        headers = {"Content-Type": "application/json"}
...
def test_agent_can_activate_and_clear_repo_map_skill(tmp_path):
    ...
    skill = agent.set_skill("repo-map")

    assert skill.name == "repo-map"
    assert agent.active_skill_name() == "repo-map"
    assert "Active skill: repo-map" in agent.prefix
    assert "repo-map" in agent.skills_text()

    assert agent.ask("Map this repository") == "Mapped."
    assert agent.last_prompt_metadata["active_skill"] == "repo-map"
    assert "Active skill: repo-map" in agent.model_client.prompts[-1]
    ...
    assert resumed.active_skill_name() == "repo-map"
    assert "Active skill: repo-map" in resumed.prefix

    resumed.clear_skill()

    assert resumed.active_skill_name() == ""
